...
import { getPackages, buyPackage } from '../services/mockApi';
    ...
    if (purchaseResult) {
        return (
            <Card className="text-center">
                <div className={`mx-auto mb-4 w-16 h-16 rounded-full flex items-center justify-center ${purchaseResult.success ? 'bg-green-100' : 'bg-red-100'}`}>
                    {purchaseResult.success ? <CheckIcon className="w-10 h-10 text-green-600" /> : <span className="text-4xl text-red-600">!</span>}
                </div>
                <h2 className={`text-2xl font-bold ${purchaseResult.success ? 'text-green-700' : 'text-red-700'}`}>{purchaseResult.success ? 'Success!' : 'Failed!'}</h2>
                <p className="text-gray-600 mt-2">{purchaseResult.message}</p>
                <Button className="mt-6" onClick={() => setPurchaseResult(null)}>
