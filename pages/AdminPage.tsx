                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                     ...
                     <div>
                        ...
                        <p className="text-xl font-bold text-green-600">KSh {Math.round(averageRevenue).toLocaleString()}</p>
                    </div>
                </div>
            </Card>
                ...
                <ul className="space-y-2">
                    {revenue.slice().reverse().map(item => (
                        <li key={item.date} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                            <span className="font-medium text-gray-700">{new Date(item.date).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</span>
