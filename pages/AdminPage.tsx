        ...
        <div className="space-y-6">
            <Card title="Revenue Dashboard">
                 <div className="text-center">
                    ...
                    <p className="text-4xl font-bold my-2 dark:text-white">KSh {latestRevenue.toLocaleString()}</p>
                 </div>
            ...
            <Card title="Weekly Performance">
                <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Total Revenue (7d)</p>
            ...
            <Card title="Daily Revenue Breakdown">
                <ul className="space-y-2">
                    {revenue.slice().reverse().map(item => (
                        <li key={item.date} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded">
                            <span className="font-medium text-gray-700 dark:text-gray-300">{new Date(item.date).toLocaleString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</span>
                            <span className="font-semibold dark:text-white">KSh {item.revenue.toLocaleString()}</span>
                        </li>
                    ))}
                </ul>
            </Card>
